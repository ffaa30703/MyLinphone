...
public class PasswordActivity extends FragmentActivity implements OnClickListener,IdelayMethodrd{
	...
	private final int WAIT_PASSWORD_RESPONSE = 12;
	private final int GET_RESPONSE_BY_THREAD = 13;
	private boolean responseDetecting = false;
	protected boolean isValidPassword = false;
	
	private  Handler mHandler=new  Handler(){
		public void handleMessage(Message msg) {
			
			if(msg.what == GET_RESPONSE_BY_THREAD){
...
//							mI2cInterface.SetDoorRelease();
						...
						if(password.equals(passwordStringBuffer.toString())){
							Intent mGeneralSetupIntent = new Intent(PasswordActivity.this,GeneralSetupActivity.class);
				...
				finish();
			}else if(msg.what == WAIT_PASSWORD_RESPONSE){
				Intent intet=new Intent(PasswordActivity.this,CheckPasswordStartActivity.class);
				Bundle bundle=new Bundle();
				bundle.putBoolean(Constant.PASSWORD_CHECKED_RESULT, false);
				bundle.putInt(Constant.ENTRY_TAG,fromActivityTag);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.wits_activity_passwordactivity);
...
//				mHandler.sendMessage(message);
					...
					Toast.makeText(PasswordActivity.this, "Please input the first password", Toast.LENGTH_SHORT).show();
				}
		}
...
//				mHandler.sendMessage(message);
					...
					Toast.makeText(PasswordActivity.this, "Please input the secornd password", Toast.LENGTH_SHORT).show();
				}
		}
		...
		class password4watch implements TextWatcher{
			...
			public void afterTextChanged(Editable s) {
					...
					mHandler.sendMessage(message);
						...
						Toast.makeText(PasswordActivity.this, "Please input the third password", Toast.LENGTH_SHORT).show();
					}
			}
			...
			if(keyCode == 19){
				   ...
				   switch (downCount) {
					...
					case 4:
						password4EditText.setText("*");
						mHandler.removeMessages(TIME_OUT);
						mHandler.sendEmptyMessageDelayed(WAIT_PASSWORD_RESPONSE,10000);
			...
			new Thread() {
				...
				public void run() {
					...
					try {
						...
						int resultResponse = mI2cInterface.GetDoorPassword(result);
						Log.d("Proximity", "GetProximityStatue.result = "+resultResponse);
						if(resultResponse == 0){
							responseDetecting = false;
							mHandler.removeMessages(WAIT_PASSWORD_RESPONSE);
